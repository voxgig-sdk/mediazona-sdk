...
import (
	...
	omni "github.com/voxgig-sdk/mediazona-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MediazonaError); is {
				out := map[string]any{
					"name":    "MediazonaError",
...
func omniCtx(arg any, client *sdk.MediazonaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MediazonaSDK); is {
			client = live
