...
require_relative 'Mediazona_types'
...
class MediazonaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MediazonaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MediazonaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MediazonaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MediazonaUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MediazonaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MediazonaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MediazonaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MediazonaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MediazonaError.new(
        "graphql_error", "MediazonaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Infographic.list / client.Infographic.load({ "id" => ... })
  def Infographic(data = nil)
    require_relative 'entity/infographic_entity'
    InfographicEntity.new(self, data)
  end
