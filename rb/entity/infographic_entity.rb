...
class InfographicEntity
  ...
  def make
    opts = @_entopts.dup
    InfographicEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MediazonaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Infographic, Hash] the current Infographic data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MediazonaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Infographic fields)
  def match_get
  ...
  #
  # @param reqmatch [InfographicListMatch, Hash, nil] match filter (any subset of
  #   Infographic fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Infographic>, Array] the matching Infographic items; raises MediazonaError on failure
