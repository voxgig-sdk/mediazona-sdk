...
module MediazonaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MediazonaResponse.new({})
    ...
    if fetch_err
      ...
      response = MediazonaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MediazonaResponse.new(fetched)
    else
