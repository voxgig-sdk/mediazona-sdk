...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MediazonaOmni.make_runner(TEST_JSON_FILE, MediazonaSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MediazonaOmni.errify(err)
    end
