...
from mediazona_sdk.utility import register

# Load features
from mediazona_sdk.feature.base_feature import MediazonaBaseFeature
from mediazona_sdk.features import _has_feature, _make_feature
...
class MediazonaSDK:
    ...
    def get_utility(self):
        return MediazonaUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("MediazonaSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Infographic(self, data=None) -> "InfographicEntity":
        """Entity factory: client.Infographic().list() / client.Infographic().load({"id": ...})."""
        from mediazona_sdk.entity.infographic_entity import InfographicEntity
        return InfographicEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "MediazonaSDK":
        if sdkopts is None:
            sdkopts = {}
