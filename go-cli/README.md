...
boru-driven command-line client **and** interactive REPL for the Mediazona
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mediazona-cli)
make build
...
./mediazona-cli list infographic

# 5. Override the API base URL for a single call
MEDIAZONA_BASE=https://api.example.com ./mediazona-cli list infographic

# 6. No arguments -> interactive REPL
./mediazona-cli
mediazona> list infographic
mediazona> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mediazona-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mediazona-cli list infographic
   ```
...
Run with no arguments to open a REPL (prompt `mediazona>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `infographic`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mediazona-cli`. |
...
(`./mediazona-cli <expr>`) and interactively (the REPL), and expressions compose the
