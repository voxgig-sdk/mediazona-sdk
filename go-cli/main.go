// Command Mediazona-cli is a CLI + REPL for the Mediazona SDK,
...
const prompt = "mediazona"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "infographic"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MEDIAZONA_APIKEY (api key), MEDIAZONA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MEDIAZONA_APIKEY carries the API key and
	// MEDIAZONA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MEDIAZONA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MEDIAZONA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMediazonaSDK(opts)
