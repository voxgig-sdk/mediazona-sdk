...
const TEST_SEED = { "entity": { "infographic": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+MediazonaSDK\b/.test(code) ||
        /\bMediazonaSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+MediazonaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/MediazonaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
