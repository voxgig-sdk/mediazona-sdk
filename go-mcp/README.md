...
[MCP](https://modelcontextprotocol.io) server exposing the Mediazona SDK as
two agent tools — `mediazona_list` and `mediazona_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mediazona-mcp)
make build

# 2. Provide credentials via the environment
export MEDIAZONA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mediazona \
  -- /absolute/path/to/mediazona-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mediazona-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mediazona_list: first page of records
{ "entity": "infographic" }
{ "entity": "infographic", "query": { } }

// mediazona_load: one record by id
{ "entity": "infographic", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mediazona-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mediazona \
     -- "$PWD"/dist/*/mediazona-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mediazona_list` and `mediazona_load` tools now appear
   in new sessions. Ask the agent to *"list infographic using mediazona"*
   and it calls `mediazona_list` with `{"entity":"infographic"}`.
...
```sh
export MEDIAZONA_APIKEY=sk_live_xxx            # API key
export MEDIAZONA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `mediazona_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mediazona_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MEDIAZONA_APIKEY` | API key sent with every request. |
| `MEDIAZONA_BASE` | Optional override of the API base URL. |
...
```sh
./mediazona-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mediazona_load","arguments":{"entity":"infographic","query":{"id":1}}}}'
```
