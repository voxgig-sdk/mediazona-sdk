...
// Mediazona SDK Infographic entity
...
class InfographicEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new InfographicEntity($this->_client, $opts);
    }

    /**
     * @param Infographic|array $args Infographic data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MediazonaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Infographic|array The current Infographic data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Infographic fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MediazonaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Infographic fields).
     */
    ...
    /**
     * List Infographic items matching the given filter.
     *
     * @param InfographicListMatch|array|null $reqmatch Match filter (any subset
     *   of Infographic fields) as an assoc-array; InfographicListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Infographic[]|array A list of Infographic items as assoc-arrays at
     *   the SDK boundary; throws MediazonaError on failure (item-5 convention).
     */
