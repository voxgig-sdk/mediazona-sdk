...
// infographics is an array of Infographic entities, populated with mock data
// — call infographics[0].data() for the record itself
...
// List all infographics (returns InfographicEntity[] — .data() for the record)
const infographics = await client.Infographic().list()
...
$infographics = $client->Infographic()->list();
print_r(array_map(fn($item) => $item->data_get(), $infographics));
```
