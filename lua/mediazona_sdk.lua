...
local MediazonaSDK = {}
MediazonaSDK.__index = MediazonaSDK
...
MediazonaSDK._make_feature = _make_feature


function MediazonaSDK.new(options)
  local self = setmetatable({}, MediazonaSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MediazonaSDK:options_map()
...
function MediazonaSDK:get_utility()
  return Utility.copy(self._utility)
end


function MediazonaSDK:get_root_ctx()
  return self._rootctx
end


function MediazonaSDK:prepare(fetchargs)
...
function MediazonaSDK:direct(fetchargs)
...
-- by passing it.
function MediazonaSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MediazonaSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MediazonaSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Infographic():list() / client:Infographic():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function MediazonaSDK:Infographic(data)
  local EntityMod = require("entity.infographic_entity")
  if data == nil then
    if self._infographic == nil then
      self._infographic = EntityMod.new(self, nil)
    end
    return self._infographic
  end
...
function MediazonaSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MediazonaSDK.new(sdkopts)
  sdk.mode = "test"
