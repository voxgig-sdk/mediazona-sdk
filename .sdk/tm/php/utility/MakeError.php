...
class MediazonaMakeError
{
    public static function call(?MediazonaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MediazonaContext([], null);
        }
        $op = $ctx->op ?? new MediazonaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MediazonaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MediazonaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MediazonaError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MediazonaError) {
