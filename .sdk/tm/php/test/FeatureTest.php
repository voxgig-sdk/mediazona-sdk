...
class FtCtrl extends MediazonaControl
{
...
class FtClient
{
    ...
    private MediazonaUtility $utility;

    public function __construct(MediazonaUtility $utility, array $options)
    {
    ...
    public function get_utility(): MediazonaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MediazonaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MediazonaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MediazonaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MediazonaConfig::make_feature($name);
    ...
    private static function buildUrl(MediazonaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MediazonaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MediazonaResponse($fetched);
        }
    ...
    private function fail(MediazonaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MediazonaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
