...
{
    private static function utility(): MediazonaUtility
    {
        return new MediazonaUtility();
    }
    ...
    private static function ctx(array $over = []): MediazonaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MediazonaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MediazonaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MediazonaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MediazonaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MediazonaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MediazonaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MediazonaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MediazonaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MediazonaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MediazonaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MediazonaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MediazonaResponse([]),
            'result' => new MediazonaResult([]),
        ]);
        [, $err] = MediazonaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MediazonaSpec([]),
            'response' => null,
            'result' => new MediazonaResult([]),
        ]);
        [, $err] = MediazonaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MediazonaSpec(['step' => 's']),
            'response' => new MediazonaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MediazonaResult([]),
        ]);
        [, $err] = MediazonaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MediazonaSpec(['step' => 's']),
            'response' => new MediazonaResponse(self::resp(200, ['v' => 1])),
            'result' => new MediazonaResult([]),
        ]);
        [, $err] = MediazonaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MediazonaSpec(['step' => 's']),
            'response' => new MediazonaResponse(self::resp(200, ['v' => 2])),
            'result' => new MediazonaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MediazonaMakeResponse::call($ctx);
    ...
    {
        $preset = new MediazonaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MediazonaSpec([]),
            'response' => new MediazonaResponse([]),
            'result' => new MediazonaResult([]),
        ...
        [$got, $err] = MediazonaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MediazonaResult([])]);
        [, $err] = MediazonaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MediazonaSpec([]), 'result' => null]);
        [, $err] = MediazonaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MediazonaSpec(['step' => 's']),
            'result' => new MediazonaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MediazonaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MediazonaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MediazonaSpec(['step' => 's']),
            'result' => new MediazonaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MediazonaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MediazonaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MediazonaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MediazonaSpec([]),
            'result' => new MediazonaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MediazonaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MediazonaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MediazonaError('boom', 'boom');
        $utility->fetcher = function (MediazonaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MediazonaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MediazonaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MediazonaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MediazonaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MediazonaResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MediazonaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MediazonaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MediazonaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MediazonaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MediazonaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MediazonaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MediazonaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MediazonaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MediazonaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MediazonaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MediazonaResult(['ok' => false])]);
        $this->expectException(MediazonaError::class);
        MediazonaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MediazonaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MediazonaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MediazonaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MediazonaBaseFeature();
        $b = new MediazonaBaseFeature();
        MediazonaFeatureAdd::call($ctx, $a);
        MediazonaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MediazonaBaseFeature
    {
        $f = new MediazonaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MediazonaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MediazonaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MediazonaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MediazonaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MediazonaResponse(['headers' => null]),
            'result' => new MediazonaResult([]),
        ]);
        MediazonaResultHeaders::call($ctx);
